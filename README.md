...
|---------|------|-------------|
| 5.7.2 | 2026-05-04 | **fix(mobile): pin chat composer to the visual viewport while the keyboard is open.** v5.7.1 hid the bottom nav-rail, but some mobile browsers kept the page layout box taller than the keyboard-shrunken visual viewport, leaving a dark blank band between the composer and the keyboard. `body.keyboard-open #chat-input-area` now switches to `position: fixed; bottom: 0`, `#chat-messages` drops the extra safe-area padding while the keyboard is visible, and the regression test asserts the nav, content, composer, and message-padding rules together. |
| 5.7.1 | 2026-05-04 | **fix(mobile): hide bottom nav-rail when soft keyboard is open.** On narrow viewports the horizontal nav-bar was rendered between the chat input and the soft keyboard, creating a blank gap. `app.js` now detects keyboard-open state via `visualViewport.height` vs `window.innerHeight` (>30% reduction) and toggles `body.keyboard-open`; a new `@media (max-width: 640px)` block in `style.css` hides `#nav-rail` and removes the `#content` bottom padding reservation while the keyboard is visible, so the chat input docks flush against the keyboard with no dead space. |
...
Older releases are preserved in Git tags and GitHub releases. The 5.2.0, 5.2.3, 5.3.x release-candidate rows, former `4.0.0` row, and v5.6.1 patch row are rolled off to respect the P9 changelog cap; their full bodies remain at their git tags.
