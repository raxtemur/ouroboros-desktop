...
[![Version 4.34.2](https://img.shields.io/badge/version-4.34.2-green.svg)](VERSION)
...
|---------|------|-------------|
| 4.34.2 | 2026-04-17 | **LAN Access IP discovery fix — VPN no longer masks the LAN address.** The previous implementation used `socket.connect("8.8.8.8")` to guess the local IP; when a VPN intercepts the default route, that call returned the VPN tunnel IP (e.g. `172.16.9.1`) instead of the real WiFi address (`192.168.0.92`). Extracted a new `_discover_lan_ips()` helper in `server.py` that prefers `socket.gethostbyname_ex(socket.gethostname())` (which typically excludes VPN tunnels on macOS/Linux), falls back to `socket.getaddrinfo` for IPv4 aliases, and only uses the UDP-connect trick as a last resort when both resolver paths return nothing. All three paths filter loopback (`127.*`) and link-local (`169.254.*`) addresses and deduplicate while preserving order. `api_network_info` now dispatches the blocking resolution to `asyncio.to_thread` and guards the UDP fallback socket with `try/finally` for deterministic cleanup — closing the two advisory findings (async-blocking and socket-cleanup) that survived the 4.34.1 commit. Tests: `tests/test_lan_access.py` refreshed to 10 tests — removed unused `importlib`/`json`/`types` imports, replaced the stale `test_api_network_info_returns_ips` fixture (which assumed UDP-trick primary), and added targeted regressions for the VPN-interception scenario, loopback/link-local filtering, deduplication, UDP fallback activation, and `asyncio.to_thread` dispatch. |
| 4.34.1 | 2026-04-17 | **LAN Access toggle in Settings.** Add `OUROBOROS_SERVER_HOST` to `SETTINGS_DEFAULTS` and `apply_settings_to_env` so server bind host is configurable via Settings. New "Allow LAN Access" checkbox in Settings → Providers → Network Gate toggles between `127.0.0.1` (localhost only) and `0.0.0.0` (all interfaces). New `/api/network-info` endpoint detects the device's LAN IP and displays a clickable `http://<ip>:<port>` hint below the checkbox when enabled. Filters out `169.254.x.x` link-local addresses. `server.py::main()` now loads settings early (before `parse_server_args`) so `OUROBOROS_SERVER_HOST` takes effect on the first bind. `OUROBOROS_SERVER_HOST` added to `_RESTART_REQUIRED_KEYS`. 7 new regression tests in `tests/test_lan_access.py`. |
