...
|---------|------|-------------|
| 5.7.4 | 2026-05-05 | **fix(mobile): use flex-column layout and visualViewport.offsetTop for keyboard-open chat.** Replaces per-element `position: fixed` with a single flex-column container on `#page-chat`, eliminating the blank gap above/below the composer. `updateVvh()` now also exposes `--vvh-offset` (from `visualViewport.offsetTop`) so the container starts at the correct visual viewport origin on iOS/Android. **Note on changelog rolloff**: the v5.6.3 patch row was rolled off to respect the P9 5-patch-row cap; its full body remains at git tag `v5.6.3`. |
| 5.7.3 | 2026-05-05 | **fix(mobile): pin chat stack to visual viewport when soft keyboard opens.** On narrow viewports (`≤640px`), `updateVvh()` in `web/app.js` now detects keyboard-open state via `(layoutHeight - visualViewport.height) > max(120px, 25%)` and toggles `body.keyboard-open`. CSS pins the chat page, header, messages, and input area to `var(--vvh)` with `position: fixed` and hides the nav rail, keeping the composer visible above the keyboard without blank gaps. Regression test added in `tests/test_chat_logs_ui.py`. **Note on changelog rolloff**: the v5.6.2 patch row was rolled off to respect the P9 5-patch-row cap; its full body remains at git tag `v5.6.2`. |
