...
- **Mobile keyboard safety**: `web/app.js` creates a `<style id="runtime-vvh">` element at startup and updates its `textContent` with `:root{--vvh:Npx}` on every `visualViewport` resize/scroll event (falling back to `window.innerHeight` for browsers without the API). `body`, `#app`, `#nav-rail`, and `.evolution-container` use `var(--vvh)` instead of `100vh`, keeping the chat input above the soft keyboard on iOS/Android and preventing the Evolution page from overflowing past the visible viewport. `--vvh` defaults to `100dvh` in `:root` so desktop layout is unchanged. In the same `visualViewport` handler, when `window.innerWidth <= 640` and the visual viewport height is more than 30% smaller than `window.innerHeight` (i.e. the soft keyboard is open), `web/app.js` adds the `keyboard-open` class to `<body>`; the class is removed when the keyboard closes. A `@media (max-width: 640px)` block in `web/style.css` reacts to `body.keyboard-open` by hiding `#nav-rail` (`display: none`), clearing `#content`'s `padding-bottom` reservation, fixing `#chat-input-area` to the visual viewport bottom, and removing the safe-area addition from `#chat-messages` while the keyboard is visible. The composer is fixed because some mobile browsers keep the page layout box taller than the keyboard-shrunken visual viewport; hiding the nav rail alone leaves an empty band below the input. The bottom nav-rail reappears automatically when the keyboard is dismissed. `renderMarkdown` wraps generated tables in `<div class="md-table-wrap">` for `overflow-x: auto` horizontal scrolling on narrow screens. On narrow viewports (`@media (max-width: 640px)`) the Costs stat/budget/tables grids and Evolution version columns collapse to single-column, and `.form-field input/select` carry `max-width: 100%` so the desktop-default `width: 320px` never overflows a narrow container.
